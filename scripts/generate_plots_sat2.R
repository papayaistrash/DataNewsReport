...
sat_salary <- data113 %>%
  mutate(
    ...
    蝮質鞈?= ? + ?鞎?
    ??摨行遛?漲 = as.numeric(q20_6),
    甈? = as.numeric(w)
  ) %>%
  filter(!is.na(蝮質鞈?, 蝮質鞈?>= 17000, 蝮質鞈?<= 40000, !is.na(??摨行遛?漲)) %>%
  mutate(
    銝遛?漲? = ??摨行遛?漲, # ???: 1=敺遛?? 5=敺?皛踵?嚗?擃?銝遛
    ?芾????= cut(蝮質鞈? 
...
p8 <- ggplot(sat_salary_summary, aes(x = ?芾????) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, 0.8),
    sec.axis = sec_axis(~ . / scale_factor, name = paste(strsplit("??摨虫?皛?, "")[[1]], collapse = "\n"))
  ) +
  labs(title = "?芾?蝯????漲??皛踵?摨衣?????,
       subtitle = "?瑟??鈭箸雿? (撌西遘)嚗?蝺??箇宏撌仿??漲?像??皛踵?摨?(?唾遘)",
       x = "蝮質鞈???(?怠??剛祥)",
